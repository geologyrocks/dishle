...
```bash
$> docker build -t dishle:dev -f docker/Dockerfile .
$> docker run -d -p 3000:3000 --name dishle-dev dishle:dev
```
...
```bash
$> docker build --target=prod -t dishle:prod -f docker/Dockerfile .
$> docker run -d -p 80:8080  --name dishle-prod dishle:prod
```
